    ...
    <main className="mx-auto max-w-7xl px-6 py-12 sm:py-16">
      ...
      <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {docSections.map((doc) => (
          <Link
            key={doc.title}
            href={doc.href}
            className="flex flex-col gap-3 rounded-lg border p-5 hover:shadow-lg transition-shadow"
          >
            <div className="flex items-center gap-3">{doc.icon}<h3 className="text-base font-medium">{doc.title}</h3></div>
      ...
      <section className="text-center mt-16">
        ...
        <p className="text-foreground/70 mb-6">
          Explore the docs, pick your tools, and integrate Moca Passport in minutes.
        </p>
